...
public class Quiz extends JFrame implements ActionListener{
    ...
    String qa[][] = new String[10][2]; // to store question number and their option dynamically we have used 2D array
    ...
    Quiz(String username){
        ...
        ImageIcon i1 = new ImageIcon(ClassLoader.getSystemResource("simple/minds1/Icon/quiz.jpeg"));
        JLabel l1 = new JLabel(i1);
        l1.setBounds(0, 0, 1440, 392); //image height width
        add(l1);
        
        qno = new JLabel("");
        qno.setFont(new Font("Tahoma", Font.PLAIN, 24)); //to change font style and size;
        qno.setBounds(100, 450, 50, 30);
        add(qno);
        
        question = new JLabel("");
        question.setFont(new Font("Tahoma", Font.PLAIN, 24)); //to change font style and size;
        ...
        opt1 = new JRadioButton("");  //radio button to select multiple options
        ...
        options = new ButtonGroup();  // this object of button group is created to select only one value of radio button since noe they
        // are group/link together it is possible to select only one value
        options.add(opt1);
        options.add(opt2);
        options.add(opt3);
        options.add(opt4);
        ...
        next.setBackground(new Color(30, 144, 255)); //set background color
        next.setForeground(Color.WHITE); //set text color
        next.addActionListener(this); //use to tell what happens when click on button;
        next.setBounds(1100, 550, 200, 40); //position of button
        add(next);
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource() == next){
            ...
            if(options.getSelection() == null){
                ...
                pa[count][0] = options.getSelection().getActionCommand(); // used to get correct answer set by setaction command
            }
            ...
            count++;
            start(count);  //calling start function again to see next question after clicking on next button
            ...
            this.setVisible(false);  //close this frame cz we want to open next frame
    ...
    public void start(int count){  //fuction to set Q&A
        qno.setText("" + (count + 1) + ". "); //to set values in label(question number)
        question.setText(q[count][0]);  //to set question
        opt1.setLabel(q[count][1]);  //to set options of question
        opt1.setActionCommand(q[count][1]);  // get action command to set correct answer
        opt2.setLabel(q[count][2]);  //to set options of question
        opt2.setActionCommand(q[count][2]);
        opt3.setLabel(q[count][3]);  //to set options of question
        opt3.setActionCommand(q[count][3]);
        opt4.setLabel(q[count][4]); //to set options of question
